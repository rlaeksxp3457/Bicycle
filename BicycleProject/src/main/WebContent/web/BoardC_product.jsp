...
<div class="row">
    ...
    <div class="col-lg-8">
        ...
        <button type="submit" class="btn btn-outline-primary"
                onclick="history.back(-1);" style="float: right;">이전
        </button>
    </div>
    <div class="col-lg-1"></div>
    <div class="col-lg-1"></div>
...
<div class="row">
    ...
    <div class="col-lg-8">
        <%
            if (group != 5) {
        %>
        <a href="BoardC_delete_action.jsp?market_id=<%=market_id%>"
           class="btn btn-outline-danger"
           style="float: right; border-radius: 20px;">삭제</a> <a
            href="BoardC_update.jsp?market_id=<%=market_id%>"
            class="btn btn-outline-danger"
            style="float: right; border-radius: 20px;">수정</a>
        <%
        } else {
        %>
        ...
        <a href="BoardC_delete_action.jsp?market_id=<%=market_id%>"
           class="btn btn-outline-danger"
           style="float: right; border-radius: 20px;">삭제</a>
        <%
            }
        %>
    </div>
                ...
                <div class="row no-gutters">
                    ...
                    <div class="col-8">
                        <div class="card-body text-center">
                            <h5 class="card-title"><%=boardcDto.getBoard_Title()%>
                            </h5>
                            <p class="card-text"><%=boardcDto.getMarket_Name()%>
                            </p>
                            <p class="card-text">
                                가격:
                                <%=boardcDto.getMarket_Price()%>원
                            </p>
                            <p class="card-text">
                                판매자:
                                <%=boardcDto.getMember_Id()%>
                            </p>
                            <p class="card-text">
                                번호: 0<%=memberDto.getMember_phone()%>
                            </p>
                            <p class="card-text">
                                지역:
                                <%=boardcDto.getMarket_Addr()%>
                            </p>
                            <p class="card-text"><%=boardcDto.getBoard_Content()%>
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <ul class="list-group">
            <li id="guest_rep_id" class="list-group-item">
                <div class="guest_rep_class">
                    <strong><%=CommentDto.getComment_name()%>
                    </strong> <small><%=CommentDto.getComment_regdate()%>
                </small>
                    <span class="control"> <%
                    ...
                    %> <a href="#"
                          onclick="guest_rep_onclick_coedit('edit','<%=CommentDto.getComment_id()%>','<%=boardcDto.getMarket_Id()%>')"
                          class="btn btn-default btn-xs"> <span>수정</span></a> <a href="#"
                    ...
                    <p><%=CommentDto.getComment_content()%>
                    </p>
                </div>
            </li>
        </ul>
    ...
    <div class="col-lg-8">
        <form action="BoardC_product_process.jsp" method="post">
            ...
            <input type="submit" value="등록" class="btn btn-outline-primary"
                   style="float: right;"/> <a class="btn btn-outline-primary "
                                              onclick="history.back(-1);" style="float: right; color: #007bff;">목록</a>
        </form>
    </div>
    <div class="col-lg-1"></div>
    <div class="col-lg-1"></div>
        ...
        <nav style="text-align: center;">
            <ul class="pagination">
                ...
                <li class="page-item active"><a
                        href="./BoardC_product.jsp?pageNum=<%=startNum + i%>"><%=startNum + i%>
                </a></li>
                <%
                } else {
                %>
                ...
                <li><a href="./BoardC_product.jsp?pageBlock=Next"
